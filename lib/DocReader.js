...
import parsers from './DefaultParsers';
				...
				paths: _(blocks)
					...
					.mapValues(b => ({
						[b.route.method]: {
							...
							...(b.params ? {parameters: b.params.map(param => {
								...
								var schemaRender = node => {
									...
									} else if (match = /^(<(?<wrapper>)>)(?<childType>.+)$/.exec(node)?.groups) { // e.g. `array<string>`
		...
		input.split(/\s*\n\s*/)
			.forEach((line, lineOffset, lines) => {
				if (/^\s*\/\*\*/.test(line)) { // Start of block
					if (inBlock) {
						debug('Opening line: ' + lineOffset, line);
						settings.warn(`Start of block when already in block at line ${lineOffset+settings.lineOffset}`);
						return;
					}
					block = {};
					...
					if (!inBlock) {
						debug('Closing line: ' + lineOffset, line);
						settings.warn(`Attempt to close non-existant block at line ${lineOffset+settings.lineOffset}`);
						return;
					}
